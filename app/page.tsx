...
export default function Home() {
  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className="max-w-5xl mx-auto px-6 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <a
            ...
            className="w-8 h-8 rounded-full bg-[#1a1a1a] flex items-center justify-center text-white text-xs hover:bg-[#C8982E] transition-colors"
          >
      ...
      {/* ─── BINGO AGENDA (special events highlight) ─── */}
        ...
        <section className="max-w-5xl mx-auto px-6 pb-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {specialEvents.map((event) => (
              <div
                key={event.id}
                className="bg-[#1a1a1a] rounded-2xl p-6 text-center text-white"
              >
        ...
        <div className="grid gap-3 max-w-2xl mx-auto">
          {upcomingEvents.length === 0 ? (
            ...
            upcomingEvents.map((event) => (
              <div
                key={event.id}
                className="bg-white border border-gray-100 hover:border-[#C8982E]/30 rounded-xl overflow-hidden flex transition-colors"
              >
                <div className="flex-shrink-0 w-20 flex flex-col items-center justify-center p-3 bg-[#1a1a1a] text-white">
                ...
                <div className="p-4 flex-1">
                  ...
                  <p className="text-xs text-gray-500">{event.location} — {event.address}</p>
                  {event.description && (
      ...
      <section id="sponsoren" className="max-w-5xl mx-auto px-6 py-20">
        ...
        {sponsors.filter((s) => s.tier === 'silver').length > 0 && (
          <div className="mb-8">
            <p className="text-center text-xs uppercase tracking-wider text-gray-400 font-semibold mb-4">Zilver</p>
        ...
        {sponsors.filter((s) => s.tier === 'bronze').length > 0 && (
          <div>
            <p className="text-center text-xs uppercase tracking-wider text-gray-400 font-semibold mb-4">Brons</p>
        ...
        <div className="max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-4 mb-10">
            {[
              ...
              <div
                key={t.tier}
                className="bg-white border border-gray-100 hover:border-[#C8982E]/30 rounded-xl p-4 text-center transition-colors"
              >
                <div className="font-semibold text-sm">{t.tier}</div>
                <div className="text-2xl font-bold gold-text my-2">{t.price}</div>
                <div className="text-xs text-gray-400">per seizoen</div>
          ...
          >
            <div>
              ...
              <input type="text" name="bedrijf" required
                className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C8982E] placeholder:text-gray-400" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Gewenst pakket</label>
              <select name="pakket"
                className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#C8982E]">
            ...
            <button type="submit"
              className="w-full bg-[#C8982E] text-white font-semibold py-3 rounded-lg hover:bg-[#B8882A] transition-colors">
      ...
      <footer className="border-t border-gray-200 py-8">
        <div className="max-w-5xl mx-auto px-6 text-center">
          ...
          <p className="text-sm text-gray-400">© 2026 Maaspoort Bingo · Alle rechten voorbehouden</p>
          <a href="#" className="text-xs text-gray-400 hover:text-[#C8982E] transition-colors">Cookiebeleid</a>
        </div>
      </footer>
    </div>
  );
...
function SponsorCard({ sponsor, small }: { sponsor: (typeof sponsors)[0]; small?: boolean }) {
  const content = (
    <div className={`bg-white border border-gray-100 hover:border-[#C8982E]/30 rounded-xl transition-colors ${small ? 'p-3' : 'p-5'}`}>
      <div className={`rounded-lg flex items-center justify-center font-bold text-gray-300 mb-2 ${small ? 'h-10 text-base' : 'h-14 text-xl'}`}>
        {sponsor.logo ? (
      ...
      <div className={`font-semibold text-gray-800 ${small ? 'text-xs' : 'text-sm'}`}>{sponsor.name}</div>
